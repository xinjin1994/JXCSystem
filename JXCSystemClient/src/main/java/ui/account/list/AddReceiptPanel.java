	...
	*//**
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource() == forwardButton){
			...
			//GetVO(String id, String cusName, String operator, TransferListVO transferList) {
			//单据编号（SKD-yyyyMMdd-xxxxx），客户（同时包含供应商和销售商），操作员（当前登录用户），转账列表
			newReceipt = new GetVO(id,cusName,operate,transferListVO);
			total.setText(accountblService.calTotalMoney_up(newReceipt)+"");
			setTotal();
			uiController.confirmReceipt(newReceipt,person,accountblService.calTotalMoney_up(newReceipt),balance);//这边将新生成的收款单信息传给确认收款单界面
